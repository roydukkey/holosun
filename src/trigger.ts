 ...
 * @internal
 */
export function trigger (target: EventTarget, types: string, options?: CustomEventInit, useCustomEvent?: boolean): Array<[string, boolean]> {

	// Disable for minimality
	// eslint-disable-next-line @typescript-eslint/prefer-optional-chain
	useCustomEvent = useCustomEvent || options !== undefined && options.detail !== undefined;

	return separateTypes(types).map((type) => [
		type,
		target.dispatchEvent(new (useCustomEvent ? CustomEvent : Event)(type, options))
	]);
